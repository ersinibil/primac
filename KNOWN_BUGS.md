...
1. **YÜKSEK — `mobile/task_view.php` IDOR** — satır 14-19: `task_status` güncellemesi
   ...
   değiştirebilir.
2. **YÜKSEK — `sifre_sifirla.php` brute-force koruması yok** — 6 haneli kod (`random_int(100000,999999)`),
   30 dakika geçerli, deneme sayısı sınırı/lockout yok.
3. **ORTA-YÜKSEK — `accounting.php` yansıyan XSS** — satır 8, 111-130: `$_GET['tab']` escape
   edilmeden href'e basılıyor (`h()`/`htmlspecialchars()` eksik). Mobilde bu parametre kullanılmıyor.
4. **ORTA — `users.php` rol yükseltme** — satır 41-51: `users` modül yetkisi olan admin-olmayan
   biri kendini/başkasını `admin` rolüne yükseltebilir, `uid===1` dışında kısıtlama yok.
5. **ORTA — `is_admin()` session'da bayatlıyor** — `boot.php:124-127`, `user_can()` gibi DB'den
   taze okumuyor; rolü alınan bir admin aktif oturumu boyunca admin yetkisiyle işlem yapabilir.
6. **ORTA — Login'de session fixation koruması yok** — `index.php:12-40`, başarılı girişte
   `session_regenerate_id(true)` çağrılmıyor.
7. **DÜŞÜK — `cron.php:7`'de yeni sabit anahtar** (`acans-cron-2026`) — `acans-migrate-2026`'dan
   farklı, daha önce raporlanmamış.
8. **BİLGİ — Proje genelinde CSRF token mekanizması yok.**
...
9. **Sabit migration/temizlik anahtarı** — `migrate.php` ve `temizle.php` aynı sabit kodlanmış
...
10. **`tasks` tablosunda kayıt kaynağı ayrımı yok** — Admin'in başkasına atadığı iş (`task_new.php`)
