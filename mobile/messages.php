...
/* P0 MOBİL HOTFIX (2026-07-19, Product Owner FAIL raporu) — composer artık global bottom nav'ın
   YERİNİ ALMAZ, nav'ın TAM ÜSTÜNE oturur. ÖNCEKİ tur (sabit calc(70px+safe-area) tahmini) gerçek
   cihazda nav'ın GERÇEK render yüksekliğiyle örtüşmüyordu (composer nav'a biniyordu) — artık
   common.php::botx()'teki ResizeObserver'ın yazdığı --acans-navh (gerçek ölçülen nav yüksekliği)
   okunuyor, sabit değer sadece JS hiç çalışmazsa devreye giren fallback. */
...
/* CHAT MODU: composer + nav ikisi de fixed — thread'in alt boşluğu ikisinin GERÇEK ölçülen toplam
   yüksekliğini (--acans-composerh + --acans-navh, bu dosyanın JS'i tarafından yazılır) karşılamalı
   ki son mesaj balonu arkalarında kalmasın. */
...
function unpinC(){ if(!composer)return; composer.style.top='auto'; composer.style.bottom='var(--acans-navh, calc(70px + env(safe-area-inset-bottom)))'; composer.style.paddingBottom=''; }
...
function unpinComposer(){
  if(!composer) return;
  composer.style.top='auto'; composer.style.bottom='var(--acans-navh, calc(70px + env(safe-area-inset-bottom)))';
