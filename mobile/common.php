...
function block_personel(){ global $isAdmin; if(!$isAdmin){ header('Location: index.php'); exit; } }
function topx($t){global $name,$isAdmin; $um=unread_msg(); $un=unread_notif(); ?><!doctype html><html lang="tr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><title><?=htmlspecialchars($t)?></title><link rel="manifest" href="manifest.php"><meta name="theme-color" content="#071326"><meta name="apple-mobile-web-app-capable" content="yes"><meta name="apple-mobile-web-app-status-bar-style" content="black"><meta name="apple-mobile-web-app-title" content="ACANS OTS"><link rel="apple-touch-icon" href="icon.php?size=180"><link rel="icon" href="icon.php?size=192"><style>
*{box-sizing:border-box}html{-webkit-text-size-adjust:100%}body{margin:0;background:#071326;color:white;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Arial,sans-serif;padding-top:env(safe-area-inset-top);padding-bottom:calc(92px + env(safe-area-inset-bottom));touch-action:manipulation;-webkit-tap-highlight-color:transparent}a,button,select,.card,.item,.nav a{touch-action:manipulation}.app{max-width:520px;margin:auto;padding:14px 16px}.top{display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;gap:10px;position:sticky;top:0;z-index:50;background:linear-gradient(180deg,#071326 70%,rgba(7,19,38,0));padding:8px 0 10px}.brand{display:flex;gap:12px;align-items:center;color:white;text-decoration:none;min-width:0}.logo{width:46px;height:46px;border-radius:16px;background:white;color:#071326;display:flex;align-items:center;justify-content:center;font-weight:1000;flex:0 0 auto}.small{color:#94a3b8;font-size:13px}h1{font-size:22px;margin:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.acts{display:flex;gap:8px;flex:0 0 auto}.pill{position:relative;background:rgba(255,255,255,.12);color:white;border-radius:14px;width:42px;height:42px;display:flex;align-items:center;justify-content:center;text-decoration:none;font-size:17px}.dot{position:absolute;top:-5px;right:-5px;background:#ef4444;color:#fff;font-size:10px;font-weight:900;min-width:18px;height:18px;border-radius:9px;display:flex;align-items:center;justify-content:center;padding:0 4px}.panel{background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.12);border-radius:22px;padding:15px;margin:12px 0}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.card{min-height:112px;border-radius:24px;padding:15px;color:#0f172a;text-decoration:none;display:flex;flex-direction:column;justify-content:space-between}.card span{font-size:29px}.blue{background:#dbeafe}.green{background:#dcfce7}.orange{background:#ffedd5}.purple{background:#ede9fe}.red{background:#fee2e2}.yellow{background:#fef3c7}.teal{background:#ccfbf1}.gray{background:#f1f5f9}.item{display:block;background:rgba(255,255,255,.1);border-radius:17px;padding:13px;margin:10px 0;color:white;text-decoration:none}.item small{color:#94a3b8}.btn{display:inline-block;background:white;color:#071326;border:0;border-radius:14px;padding:12px 14px;text-decoration:none;font-weight:1000}.btn.dark{background:#2563eb;color:#fff}input,select,textarea{width:100%;border:0;border-radius:14px;padding:12px;font-size:16px;margin:6px 0 12px}.ok,.notice{background:#dcfce7;color:#166534;padding:12px;border-radius:14px}.err{background:#fee2e2;color:#991b1b;padding:12px;border-radius:14px}.muted{color:#94a3b8}.bottom{position:fixed;left:0;right:0;bottom:0;background:#071326;border-top:1px solid rgba(255,255,255,.12);padding:6px 8px calc(10px + env(safe-area-inset-bottom));z-index:1000}.nav{max-width:520px;margin:auto;display:grid;grid-template-columns:repeat(5,1fr);gap:4px}.nav a{position:relative;display:block;text-align:center;color:#cbd5e1;text-decoration:none;font-weight:900;font-size:12.5px;padding:9px 2px;border-radius:14px}.nav a:active{background:rgba(59,130,246,.28)}.nav span{display:block;font-size:26px;margin-bottom:2px;line-height:1}.nav .nd{position:absolute;top:-2px;right:14px;background:#ef4444;color:#fff;font-size:10px;font-weight:900;min-width:17px;height:17px;border-radius:9px;display:flex;align-items:center;justify-content:center}body.chat-mode .bottom{display:none}body.chat-mode{padding-bottom:0}body.kb .bottom{display:none!important}body.kb{padding-bottom:env(safe-area-inset-bottom)}
</style></head><body><div class="app"><div class="top"><a href="javascript:void(0)" id="backbtn" onclick="if(history.length>1){history.back();}else{location.href='index.php';}" style="flex:0 0 auto;width:40px;height:42px;background:rgba(255,255,255,.12);border-radius:13px;display:flex;align-items:center;justify-content:center;font-size:26px;font-weight:900;color:#fff;text-decoration:none;margin-right:2px">‹</a><a class="brand" href="index.php"><div class="logo"><img src="icon.php?size=96" alt="ACANS" style="width:100%;height:100%;object-fit:contain;border-radius:inherit"></div><div style="min-width:0"><h1><?=htmlspecialchars($t)?></h1><div class="small"><?=htmlspecialchars($name)?> · <?=$isAdmin?'Yönetici':'Personel'?></div></div></a><div class="acts"><a class="pill" href="messages.php">💬<i class="dot" data-badge="msg" style="<?=$um?'':'display:none'?>"><?=$um?($um>9?'9+':$um):''?></i></a><a class="pill" href="notifications.php">🔔<i class="dot" data-badge="notif" style="<?=$un?'':'display:none'?>"><?=$un?($un>9?'9+':$un):''?></i></a></div></div><?php }
...
<script>/* Ana sayfada geri butonunu gizle */(function(){var b=document.getElementById('backbtn');if(b&&(/(^|\/)index\.php$/.test(location.pathname)||/\/mobile\/?$/.test(location.pathname)))b.style.display='none';})();
/* Klavye açıkken alt menüyü gizle → sayfa zıplamasın, uygulama içinde yazılsın */(function(){function f(e){return e&&(e.tagName==='INPUT'||e.tagName==='TEXTAREA');}document.addEventListener('focusin',function(e){if(f(e.target)){document.body.classList.add('kb');setTimeout(function(){try{e.target.scrollIntoView({block:'center',behavior:'smooth'});}catch(_){}}, 280);}});document.addEventListener('focusout',function(e){if(f(e.target))setTimeout(function(){var a=document.activeElement;if(!f(a))document.body.classList.remove('kb');},150);});})();</script>
<div id="acans-toast" style="position:fixed;top:-160px;left:50%;transform:translateX(-50%);width:92%;max-width:480px;background:#1e293b;border:1px solid rgba(255,255,255,.18);border-radius:16px;padding:12px 14px;box-shadow:0 16px 40px rgba(0,0,0,.5);z-index:9999;transition:top .35s ease;display:flex;gap:10px;align-items:center;cursor:pointer">
...
(function(){
  ...
  function showToast(title,body,url){
    ...
    toastTimer=setTimeout(function(){toast.style.top='-160px';},4500);
  }
