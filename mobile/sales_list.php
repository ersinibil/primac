<?php
/* SATIŞ OPERASYON MERKEZİ — Satışlar listesi (mobil, 2026-07-19, P0-2). Web sales_list.php ile
 * AYNI veri kaynağı/mantık — bkz. o dosyadaki mimari not. */
...
topx('Satışlar');
?>
<a class="df-btn df-btn--primary df-btn--lg" href="sales.php" style="width:100%;justify-content:center;margin-bottom:10px"><?=ds_icon('plus',16)?> Yeni Satış</a>

<?php if(!empty($err)): ?><?=ds_alert('danger',$err)?><?php endif; ?>
<?php if(isset($_GET['deleted'])): ?><?=ds_alert('success','Satış geri alındı.')?><?php endif; ?>
...
<form method="get" style="margin-top:10px">
  <input type="text" name="q" value="<?=h($q)?>" placeholder="cari, açıklama" style="margin-bottom:8px">
  <?php
  ...
  <?php if($q||$cid||$from||$to||$status): ?><a class="df-btn df-btn--secondary" href="sales_list.php" style="width:100%;justify-content:center;margin-top:6px">Temizle</a><?php endif; ?>
</form>
</details>

<?php foreach($rows as $r): $isDoc=!empty($r['document_id']); ?>
<a href="<?= $isDoc ? '../trade_document_view.php?id='.(int)$r['document_id'].'&web=1' : 'sale_view.php?id='.(int)$r['id'] ?>" class="df-panel" style="display:block;margin-top:10px;text-decoration:none;color:inherit">
  <div style="display:flex;justify-content:space-between;gap:8px;align-items:flex-start">
    <div class="df-list-row-title" style="color:var(--df-success-ink)"><?=mm($r['amount'])?></div>
    <?=ds_badge($r['status'])?>
  </div>
...
<?php endforeach; ?>
<?php if(!$rows): ?><?=ds_empty_state('Kayıt yok.', null, ds_icon('tag',20))?><?php endif; ?>
...
<a class="df-btn <?=$p===$page?'df-btn--primary':'df-btn--secondary'?> df-btn--sm" href="sales_list.php?<?=http_build_query($__qs)?>"><?=$p?></a>
<?php endfor; ?>
</div>
<?php endif; ?>
