<?php
/* SATIŞ OPERASYON MERKEZİ — Satış Detayı (2026-07-19, P0-2). Sadece BELGESİZ (hızlı) satışlar için
 * — document_id doluysa trade_document_view.php zaten tam bir detay/aksiyon ekranı, buraya hiç
 * girmeden oraya yönlendirilir (mükerrer ekran YOK). İşlem zinciri (stok-/cari borç+/tahsilat)
 * DEĞİŞTİRİLMEDİ — bu sayfa sadece var olan stock_reverse_sale()/stock_can_edit_sale() üzerinden
 * görünürlük+aksiyon sağlıyor. */
...
if($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['delete_sale'])){
    try{
        ...
        if($res['ok']){ header('Location: sales_list.php?deleted=1'); exit; }
        $_SESSION['sale_er']=$res['message'];
    }catch(Throwable $e){ $_SESSION['sale_er']=$e->getMessage(); }
    header('Location: sale_view.php?id='.$id); exit;
}
...
if($sale && !empty($sale['document_id'])){
    header('Location: trade_document_view.php?id='.(int)$sale['document_id']); exit;
}
...
if(!$sale){
    ...
    echo ds_button('Satışlar Listesi','sales_list.php','secondary','','',true);
...
$__actions = ds_button('Satışlar','sales_list.php','secondary','','',true);
if($sale['contact_id']) $__actions .= ds_button('Cari Profil','contact_view.php?id='.(int)$sale['contact_id'],'secondary','','',true);
if($elig['editable']) $__actions .= ds_button('✏️ Düzenle','sales.php?edit_id='.$id,'secondary','','',true);
ds_page_header('Satış #'.$id, ds_icon('tag',24), '', $__actions, false, true);
?>

<?php if(!$elig['editable'] && $elig['reason'] && can_edit_delete()): ?><?=ds_alert('info',$elig['reason'])?><?php endif; ?>

<div class="df-stat-row">
<div class="df-stat"><span>Cari</span><strong><?=h($sale['cname'] ?: '—')?></strong></div>
<div class="df-stat"><span>Tarih</span><strong><?=h($sale['movement_date'])?></strong></div>
<div class="df-stat"><span>Tutar</span><strong><?=money($sale['amount'])?></strong></div>
<div class="df-stat"><span>Durum</span><strong><?=ds_badge($sale['status'])?></strong></div>
...
// bulgusu 2026-07-19). sales.php'nin AYNI amaçlı bilgi kutusuyla aynı desen: elle df-alert div'i. ?>
<div class="df-alert df-alert--info" style="margin-bottom:var(--df-space-3)">
Bu satış tahsilat yapmadı — cariye açık borç olarak kaydedildi (kasa/banka etkilenmedi). Ödeme
<a href="finance_new.php?direction=in&contact_id=<?=(int)$sale['contact_id']?>">Tahsilat ekranından</a> ayrıca girilir.
</div>

<section class="df-card" style="margin-top:var(--df-space-4)">
<h2 style="font-size:var(--df-type-section-size);margin:0 0 var(--df-space-3)">Satış Kalemleri (BU İŞLEMİN ETKİSİ: stok −)</h2>
...
<section class="df-card" style="margin-top:var(--df-space-4)"><h2 style="font-size:var(--df-type-section-size);margin:0 0 8px">Açıklama</h2><p><?=nl2br(h($sale['description']))?></p></section>
<?php endif; ?>

<?php if(can_edit_delete()): ?>
<div class="df-card" style="margin-top:var(--df-space-4);display:flex;justify-content:flex-end">
<form method="post" onsubmit="return confirm('Bu satış geri alınacak: stok geri eklenir, cari borcu tersleşir. Emin misiniz?')">
<input type="hidden" name="delete_sale" value="1">
<button class="df-btn df-btn--danger df-btn--sm" type="submit">↩️ Satışı Geri Al</button>
</form>
</div>
<?php endif; ?>
